...
/*convierte_data: lee los datos de el archivo de prediccion(.predic)
  y lo separa en dos archivos (.0 y .1) que contienen los puntos predichos en cada clase
...
int convierte_data(char *filename){
  ...
  for(k=0;k<PTEST;k++){
    ...
    if(test[k][N1+1]<0.5)  salida=f0;      /*clase 0*/
    else                   salida=f1;      /*clase 1*/

    /*grabar el patron en el archivo elegido*/
    for(i=1;i<=N1+N3;i++) fprintf(salida,"%f\t",test[k][i]);
    fprintf(salida,"\n");
